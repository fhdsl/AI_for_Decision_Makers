...
In the previous chapter, we have started to think about the ethics of using relevant and unbiased data for building our AI model. In this chapter we will see that even if our data is unbiased and represents our population of interest, issues can still happen if the data is mishandled during the AI model building process. Let's take a look at how that can happen.

## Population and sample

The data we collect to train our model is always a limited representation of what we want to study, and as we explored in the previous chapter, bias can arise through our choice of selection. Let us define two terms commonly used in artificial intelligence and statistics research: the **population** is the entire group of entities we want to get information from, study, and describe. If we were building an artificial intelligence system to classify dog photographs based on their breeds, then the population is every dog photograph in the world. That’s prohibitively expensive and not easy data to acquire, so we use a **sample**, which is a subset of the population, to represent our desired population.

When our sample is a random subset of our population, we say that our data is unbiased and represent our population of interest. However, even if we are sure that the sample is representative of the population, a different type of bias and harm from the data can arise. It has to do with how we use the sample data for training and evaluating the model.

## Training data

```{r, fig.align="center", fig.alt = "video", echo=FALSE, out.width="90%"}
knitr::include_url("https://docs.google.com/presentation/d/11oUc4KvmSiQBCj8rzj9v_e5te62qBIyriFHG_hQvFZA/edit#slide=id.g262d5f57190_0_22")
```

The above image depics some of our samples for building an artificial intelligence model to classify dog photographs based on their breeds. Each dog photograph has a corresponding label that gives the correct dog breed, and the goal of the model training process is to have the artificial intelligence model learn the association between photograph and dog breed label. For now, we will use *all of our samples for training the model*. The data we use for model training is called the **training data**. Then, once the model is trained and has learned the association between photograph and dog breed, model will be able make new predictions: given a new dog image without its breed label, the model will make a prediction what its breed label is. 

## Testing data

```{r, fig.align="center", fig.alt = "video", echo=FALSE, out.width="90%"}
knitr::include_url("https://docs.google.com/presentation/d/11oUc4KvmSiQBCj8rzj9v_e5te62qBIyriFHG_hQvFZA/edit#slide=id.g262d5f57190_0_28")
```
...
```{r, fig.align="center", fig.alt = "video", echo=FALSE, out.width="90%"}
knitr::include_url("https://docs.google.com/presentation/d/11oUc4KvmSiQBCj8rzj9v_e5te62qBIyriFHG_hQvFZA/edit#slide=id.g262d5f57190_0_38")
```
...
```{r, fig.align="center", fig.alt = "video", echo=FALSE, out.width="90%"}
knitr::include_url("https://docs.google.com/presentation/d/11oUc4KvmSiQBCj8rzj9v_e5te62qBIyriFHG_hQvFZA/edit#slide=id.g262d5f57190_0_57")
```
...
```{r, fig.align="center", fig.alt = "video", echo=FALSE, out.width="90%"}
knitr::include_url("https://docs.google.com/presentation/d/11oUc4KvmSiQBCj8rzj9v_e5te62qBIyriFHG_hQvFZA/edit#slide=id.g262d5f57190_0_65")
```
...
```{r, fig.align="center", fig.alt = "video", echo=FALSE, out.width="90%"}
knitr::include_url("https://docs.google.com/presentation/d/11oUc4KvmSiQBCj8rzj9v_e5te62qBIyriFHG_hQvFZA/edit#slide=id.g262d5f57190_0_75")
```
...
This seemingly small tweak in how data is partitioned during model training and evaluation can have a large impact on how artificial intelligence systems are evaluated. We always should have independence between training and testing data so that our model accuracy is not inflated. There are clever ways of creating independence between training and testing data, such as a method called "K-Fold validation" that optimizes the training and testing data used while keeping them independent.

If we don't have this independence of trianinga and testing data, many real-life promotions of artificial intelligence may be overhyped. Imagine that someone claimed that their cancer diagnostic model from a blood draw is 90%. But their testing data is a subset of their training data. That would over-inflate their model accuracy, and it will less accurate than advertised when used on new patient data. Doctors would make clinical decisions on inaccurate information and potentially create harm. 
