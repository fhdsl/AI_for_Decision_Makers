...
In the previous chapter, we started to think about the ethics of using relevant and unbiased data for building our AI model. In this chapter we will see that even if our data is inclusive and represents our population of interest, issues can still happen if the data is mishandled during the AI model building process. Let's take a look at how that can happen.
...
Even if we are sure that the sample is representative of the population, a different type of bias and harm from the data can arise. It has to do with how we use the sample data for training and evaluating the model.
